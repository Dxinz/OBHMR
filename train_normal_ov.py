...
def train_one_epoch(epoch, model, loader, writer, log_freq, plot_freq):
    ...
    for step, (input) in enumerate(loader):
        ...
        mask_gt = input['gt_mask']
        # print("mask_gt:",mask_gt)
        # pts1ov = np.transpose(pts1, (0, 2, 1))
        # pts2ov = np.transpose(pts2, (0, 2, 1))
        if torch.cuda.is_available():
            pts1 = pts1.cuda().float()
            pts2 = pts2.cuda().float()
            # pts1ov = pts1ov.cuda().float()
            # pts2ov =  pts2ov.cuda().float()
            # pts1ov = torch.from_numpy(pts1ov).cuda().float()
            # pts2ov = torch.from_numpy(pts2ov).cuda().float()
        ...
        loss1, r_err, t_err, rmse, mask, mask_idx, threshold = model(pts1, pts2, pts1_normal, pts2_normal, T_gt, scale)
        # print("mask:", mask)
        # print("threshold:", threshold)
        mask_gt_cp = torch.where(mask_gt == 0.0, -1.0, 1.0)
        loss2 = loss_fn(mask - threshold, mask_gt_cp.cuda())
...
def train_one_epoch2(epoch, model, loader, writer, log_freq, plot_freq):
    model.train()
...
def eval_one_epoch(epoch, model, loader, writer, global_step, plot_freq):
    ...
    for step, (input) in enumerate(tqdm(loader, leave=False)):
        pts1, pts2 = input['pcd1'], input['pcd2']#b*n*3
        pts1_normal, pts2_normal = input['pcd1_normal'], input['pcd2_normal']
        T_gt, scale = input['transform_gt'], input['scale']
        mask_gt = input['gt_mask']
...
def eval_one_epoch2(epoch, model, loader, writer, global_step, plot_freq):
    model.eval()
...
if __name__ == "__main__":
    ...
    # general
    # parser.add_argument('--data_file', type=str, def0ault='/root/autodl-tmp/Data/modelnet40_ply_hdf5_2048')
    ...
    # args.exp_name = str(32-200-70-45-50-70)
    _init_(args)
